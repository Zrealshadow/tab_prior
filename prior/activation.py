...
def register_activation(name: str):
    """Decorator to register an activation class."""
    def decorator(cls):
...
def get_activation(name: str):
    ...
    Args:
        name: Activation name (e.g., 'relu', 'tanh', 'random_function')
              Or 'random' to sample from get_activations()

    Returns:
        Callable that returns an activation module instance.
    """
    if name == 'random':
        import random
    ...
    if name not in _ACTIVATION_REGISTRY:
        raise ValueError(f"Unknown activation: '{name}'. Available: {list(_ACTIVATION_REGISTRY.keys()) + ['random']}")

    return _ACTIVATION_REGISTRY[name]


def get_registered_activations() -> Dict[str, Type[nn.Module]]:
    """Get all registered activation classes."""
    return _ACTIVATION_REGISTRY.copy()


# Register PyTorch built-in activations
register_activation('relu')(nn.ReLU)
register_activation('tanh')(nn.Tanh)
register_activation('sigmoid')(nn.Sigmoid)
register_activation('leaky_relu')(nn.LeakyReLU)
register_activation('elu')(nn.ELU)
register_activation('gelu')(nn.GELU)
register_activation('silu')(nn.SiLU)
register_activation('softplus')(nn.Softplus)
register_activation('relu6')(nn.ReLU6)
register_activation('hardtanh')(nn.Hardtanh)
register_activation('selu')(nn.SELU)
register_activation('identity')(nn.Identity)
...
@register_activation('random_freq_sine')
...
@register_activation('random_function')
...
@register_activation('random_scale')
...
ACTIVATIONS = _ACTIVATION_REGISTRY
